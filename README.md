...
调用 `docs-seed direct-record --source agent-direct-write <commit-hash...>`，幂等地补写
...
结果文档共同判断是否跳过。只有“存档点已记录且结果文档也包含该 commit hash”的提交才会跳过；
如果结果文档已有记录但存档点缺失，Docs Seed 会补写存档点。
